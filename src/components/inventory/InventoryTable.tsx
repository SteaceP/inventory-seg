...
interface InventoryTableProps {
    ...
    fetchServerRows?: (opts: { page: number; pageSize: number; search?: string; sortField?: string; sortDir?: 'asc' | 'desc' }) => Promise<{ rows: InventoryItem[]; total: number }>;
    searchQuery?: string;
    isDesktop?: boolean;
}
...
const InventoryTable: React.FC<InventoryTableProps> = ({
    ...
    const columns: GridColDef[] = [
        {
            ...
            renderCell: (params: GridRenderCellParams) => (
                params.value ? (
                    <Box component="img" src={params.value} sx={{ width: compactView ? 36 : 48, height: compactView ? 36 : 48, objectFit: 'cover', borderRadius: 1 }} />
                ) : (
                    <Box sx={{ width: compactView ? 36 : 48, height: compactView ? 36 : 48, bgcolor: 'action.hover', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 1 }}>
        ...
        { field: 'stock', headerName: t('table.stock'), width: 120, type: 'number' },
        {
            field: 'actions',
            ...
            renderCell: (params: GridRenderCellParams) => {
                ...
                return (
                    <Box sx={{ display: 'flex', gap: 1 }}>
                        <IconButton size="small" onClick={() => onEdit(item)} sx={{ color: 'primary.main' }} title={t('inventory.edit')}>
                            <EditIcon fontSize="small" />
                        </IconButton>
                        {onDelete && (
                            <IconButton size="small" onClick={() => onDelete(id)} sx={{ color: 'error.main' }} title={t('inventory.delete')}>
    ...
    const rowSelectionModel = useMemo(() => ({ ids: new Set(Array.from(selectedItems)), type: 'include' as const }), [selectedItems]);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            setLoading(true);
            try {
                const sortItem = (sortModel && sortModel.length > 0) ? sortModel[0] : undefined;
                ...
                console.error('Failed to load server rows', err);
            } finally {
    ...
    }, [fetchServerRows, paginationModel.page, paginationModel.pageSize, serverSide, sortModel, searchQuery]);
    ...
    useEffect(() => {
        ...
        const computeAndSet = () => {
            ...
            const available = Math.max(200, height - toolbarHeight - footerHeight - headerRowHeight);
            const rowHeight = compactView ? 36 : 52;
            const newPageSize = Math.max(1, Math.floor(available / rowHeight));
            setPaginationModel((p) => (p.pageSize === newPageSize ? p : { ...p, pageSize: newPageSize }));
        };
    ...
    return (
        <Paper sx={{ width: '100%', borderRadius: compactView ? 1 : 2, overflow: 'hidden' }}>
            <div ref={containerRef} style={{ height: isDesktop ? (compactView ? 720 : 820) : (compactView ? 420 : 520), width: '100%' }}>
                <DataGrid
                    ...
                    onRowSelectionModelChange={(model: unknown) => {
                        ...
                        if (Array.isArray(model)) {
                            ...
                            const m = model as { ids: Set<string | number> | Array<string | number> };
